	...
	var socket = net.connect({
		...
		bramqp.initializeSocket(socket, function(error, handle) {
			async.series([ function(seriesCallback) {
				...
				var noWait = false;
				var arguments = {};
				handle.exchange.declare(1, exchangeName, exchangeType, passive, durable, autoDelete, internal, noWait, arguments);
				...
				handle.on('queue.declare-ok', function(channel, method, data) {
					...
					handle.on('queue.bind-ok', function(channel, method, data) {
					...
					handle.on('basic.consume-ok', function(channel, method, data) {
