	...
	specification.fetchSpecification(specPath, function(error, specification) {
		...
		self.socket.once('data', function(data) {
			...
			if (data.slice(0, 4) === 'AMQP') {
				var errorMessage = 'Server does not support AMQP version '
					+ self.specData.amqp.major + '-'
					+ self.specData.amqp.minor + '-'
					+ self.specData.amqp.revision + '. '
					+ 'The server suggests version '
					+ data[5] + '-'
					+ data[6] + '-'
					+ data[7] + '. ';
		...
		self.socket.write('AMQP' + String.fromCharCode(
			0,
			self.specData.amqp.major,
			self.specData.amqp.minor,
			self.specData.amqp.revision
...
ConnectionHandle.prototype.initializeFrameParserListeners = function() {
	...
	self.frameParser.on('header', function(channel, className, size, properties) {
		...
		if (self.frameBodyBuffer[channel].used === self.frameBodyBuffer[channel].length) {
			...
			self.emit(channel + ':' + 'content', channel, self.frameBodyClass[channel],
					self.frameBodyProperties[channel], self.frameBodyBuffer[channel]);
		}
	});
	...
	async.series([
			...
			function(seriesCallback) {
				self.once('connection.tune', function(channel, method, data) {
					...
					self.connection['tune-ok'](
						data['channel-max'] ? data['channel-max'] : 64,
						data['frame-max'],
