...
class FrameSerializer extends Transform {
	constructor(spec) {
		super({
		...
		this.frameLookup['method'] = this.serializeFrameMethod;
		this.frameLookup['header'] = this.serializeFrameContentHeader;
		this.frameLookup['body'] = this.serializeFrameContentBody;
		this.frameLookup['heartbeat'] = this.serializeFrameHeartbeat;
	...
	serializeFields(buffer, fields, data, className, methodName) {
		...
		for (let field of fields) {
			...
			for (let j in assertions) {
				checkAssertion(data[field.name], assertions[j], field, className, methodName);
			}
			if (baseDomain === 'bit') {
	...
	serializeFrameMethod(buffer, { channel, "class": theClass, className, method, methodName, data }) {
		const methodPayload = buffer.slice(7);
		methodPayload.used = 0;
		theClass = theClass || this.classLookup[className];
		method = method || this.classLookup[theClass.name][methodName];
		this.serializeShort(methodPayload, theClass.index);
		this.serializeShort(methodPayload, method.index);
		this.serializeFields(methodPayload, method.field || [], data, theClass.name, method.name);
	...
	serializeFrameContentHeader(buffer, { channel, "class": theClass, className, size, properties }) {
		...
		this.serializeLongLong(headerPayload, size);
		const fieldsPresent = [];
		const propertiesPresent = [];
		for (let fieldIndex in theClass.field) {
			const field = theClass.field[fieldIndex];
			propertiesPresent.push(properties[field.name] ? true : false);
		...
		this.serializeFields(headerPayload, fieldsPresent, properties, theClass.name, 'content');
	...
	serializeFrameContentBody(buffer, { channel, body }) {
		this.serializeFrameStart(buffer, parseInt(this.constantLookup['frame-body'].value, 10), channel, body.used);
		this.serializeFramePayload(buffer, body);
		this.serializeFrameEnd(buffer);
		buffer = buffer.slice(0, buffer.used);
		const contentBuffer = Buffer.concat([buffer.slice(0, 7), body, buffer.slice(-1)]);
	...
	_transform(data, encoding, callback) {
		let frameBuffer = Buffer.alloc(this.frameMax);
		frameBuffer.used = 0;
		frameBuffer = this.frameLookup[data.frame].call(this, frameBuffer, data);
		callback(null, frameBuffer.slice(0, frameBuffer.used));
	}
}
Object.assign(module.exports, { FrameSerializer });
