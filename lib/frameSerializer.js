...
FrameSerializer.prototype.serializeLongLong = function(buffer, longLong) {
	if (longLong !== undefined) {
		...
		if (typeof longLong === 'string') {
			longLongString = longLong;
		} else {
			longLongString = longLong.toString(16);
		}
		if (longLongString.length > 16) {
			longLongString = 'ffffffffffffffff';
		}
		while (longLongString.length < 16) {
			longLongString = '0' + longLongString;
		}
		buffer.used += buffer.write(longLongString, buffer.used, 'hex');
	} else {
...
FrameSerializer.prototype.serializeSignedLongLong = function(buffer, longLong) {
	if (longLong !== undefined) {
		...
		if (longLongString.charAt(0) === '-') {
			longLongString = longLongString.substring(1);
			var start = buffer.used;
			this.serializeLongLong(buffer, longLongString);
			for ( var i = 0; i < 8; i++) {
				buffer[start + i] = (~buffer[start + i]) & 0xff;
			}
			var lower = buffer.readUInt32BE(start + 4);
			var higher = buffer.readUInt32BE(start);
			lower += 1;
			if (lower > 0xffffffff) {
				lower = 0;
				higher += 1;
			}
			buffer.writeUInt32BE(lower, start + 4);
			buffer.writeUInt32BE(higher, start);
		} else {
			this.serializeLongLong(buffer, longLong);
		}
	} else {
...
FrameSerializer.prototype.serializeTimestamp = function(buffer, timestamp) {
	if (timestamp !== undefined) {
		this.serializeSignedLongLong(buffer, timestamp.getTime() / 1000);
	} else {
