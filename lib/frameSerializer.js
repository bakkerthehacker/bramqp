...
class FrameSerializer extends EventEmitter {
	...
	serializeFrameContentHeader(buffer, channel, className, bodySize, fieldData) {
		...
		var propertyFlags = [];
		for (var propertiesPresentIndex in propertiesPresent) {
			var localIndex = propertiesPresentIndex % 15;
			var propertyIndex = (propertiesPresentIndex - localIndex) / 15;
			if (localIndex === 0) {
