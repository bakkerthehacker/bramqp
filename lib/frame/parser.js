...
class FrameParser extends EventEmitter {
	constructor(spec) {
		super();
		this.setMaxListeners(EventEmitter.defaultMaxListeners + 100);
		this.specData = spec;
		this.parsedFrames = {};
		this.parseLookup = {};
		this.domainLookup = {};
	...
	parseFrameMethod(buffer) {
		...
		this.emit('0:heartbeat');
		this.emit(this.frameChannel + ':method', { channel: this.frameChannel, className, method, fieldData });
	}
	parseFrameContentHeader(buffer) {
		...
		const className = this.classLookup[classId].name;
		this.emit('0:heartbeat');
		this.emit(this.frameChannel + ':header', { channel: this.frameChannel, className, bodySize, properties });
	}
	parseFrameContentBody(buffer) {
		this.emit('0:heartbeat');
		this.emit(this.frameChannel + ':body', { channel: this.frameChannel, buffer });
	}
